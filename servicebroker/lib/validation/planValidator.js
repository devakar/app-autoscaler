...
var plan = require(path.join(__dirname,'../../config/plan.json'));
...
var valueInPlan = function(plan_id, key){
    for (var i = 0; i < plan.plans.length; i++){
        if(plan.plans[i].id == plan_id){
            return plan.plans[i][key]
        }
    }
}
...
var validatePlanJSONValues = function(policyJson, service_id, plan_id) {
    ...
    if(typeof policyJson !== 'undefined' && policyJson !== null && typeof policyJson.schedules !== 'undefined' && policyJson.schedules !== null ){
        if( (typeof policyJson.schedules.recurring_schedule !== 'undefined' && policyJson.schedules.recurring_schedule !== null) && (policyJson.schedules.recurring_schedule.length > valueInPlan(plan_id, "recurring_schedule_count")) ){
        ...
        if( (typeof policyJson.schedules.specific_date !== 'undefined' && policyJson.schedules.specific_date !== null) && (policyJson.schedules.specific_date.length > valueInPlan(plan_id, "specific_date_count")) ){
    ...
    if(typeof policyJson !== 'undefined' && policyJson !== null && typeof policyJson.scaling_rules !== 'undefined' && policyJson.scaling_rules !== null && (policyJson.scaling_rules.length > valueInPlan(plan_id, "scaling_rules_count")) ){
